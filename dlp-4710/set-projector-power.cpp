...
namespace {
    ...
    bool WriteCommandToProjector( int const fd, char const* format, Args... args ) {
        ...
        if ( rc < 0 ) {
            perror( "WriteCommandToProjector: write" );
            return false;
    ...
    bool ReadResponseFromProjector( int const fd, char* buffer, size_t const bufferLength ) {
        ...
        while ( index < bufferLength ) {
            ...
            if ( rc < 0 ) {
                perror( "ReadResponseFromProjector: read" );
                return false;
            } else if ( rc == 0 ) {
...
int main( int argc, char** argv ) {
    ...
    if ( argc > 1 ) {
        if ( 0 == strcmp( "--first-time", argv[1] ) ) {
            mode = Mode::FirstTimeConfiguration;
        } else if ( 0 == strcmp( "--monitor", argv[1] ) ) {
            mode = Mode::MonitorPowerAndTemperature;
        } else if ( StringToUnsignedLong( argv[1], 10, &powerLevel ) ) {
            if ( powerLevel > 1023 ) {
                fprintf( stderr, "set-projector-power: brightness range is 0..1023\n" );
                return 1;
            }
            mode = Mode::SetPowerLevel;
        } else {
            fprintf( stderr,
                "Usage: set-projector-power [--first-time|--monitor|<value>\n" //" [<duration>]]\n"
                "Where: --first-time    perform first-time projector configuration\n"
                "       --monitor       monitor projector brightness and temperature\n"
                "       <value>         brightness, in range 0..1023\n"
                //"       <duration>      milliseconds, in range 1..65535\n"
            );
            return 1;
        }

        //if ( argc > 2 ) {
        //    if ( StringToUnsignedLong( argv[2], 10, &duration ) ) {
        //        if ( duration > 65535 ) {
        //            fprintf( stderr, "set-projector-power: duration range is 0..65535\n" );
        //            return 1;
        //        }
        //    }
        //}
    ...
    switch ( mode ) {
        ...
        case Mode::MonitorPowerAndTemperature: {
            unsigned  checksum;
            unsigned  brightness;
        ...
        case Mode::SetPowerLevel:
            if ( 0 == powerLevel ) {
                ...
                //if ( ( duration > 0 ) && !SendCommand( fd, buf, "WT+LEDT=%lu", duration ) ) {
